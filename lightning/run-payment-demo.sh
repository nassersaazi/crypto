...
echo -n "Check Alice's route to Chan: "
...
run-in-node Alice "cli queryroutes --dest \"${chan_address}\" --amt 10000" > /dev/null 2>&1 \
&& {
	echo "Alice has a route to Chan"
} || {
	echo "Alice doesn't yet have a route to Chan"
	echo "Waiting for Alice graph sync. This may take a while..."
	wait-for-node Alice "cli describegraph | jq -e '.edges | select(length >= 1)'"
	echo "- Alice knows about 1 channel"
	wait-for-node Alice "cli describegraph | jq -e '.edges | select(length >= 2)'"
	echo "- Alice knows about all 2 channels"
	echo "Alice knows about all the channels"
}

echo "======================================================"
echo
echo "Get 10k sats invoice from Chan"
chan_invoice=$(run-in-node Chan "cli addinvoice 10000 | jq -r .payment_request")
echo "- Chan invoice: "
echo ${chan_invoice}

echo "======================================================"
echo
echo "Attempting payment from Alice to Chan"
run-in-node Alice "cli payinvoice --json --force ${chan_invoice} | jq -e '.failure_reason == \"FAILURE_REASON_NONE\"'" > /dev/null && {
	echo "Successful payment!"
} ||
{
	echo "Payment failed"
